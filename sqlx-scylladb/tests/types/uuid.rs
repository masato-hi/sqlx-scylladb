...
async fn it_can_select_uuid_optional(pool: ScyllaDBPool) -> anyhow::Result<()> {
    ...
    let (my_id, my_uuid, my_uuid_list, my_uuid_set): (
        Uuid,
        Option<Uuid>,
        Option<Vec<Uuid>>,
        Vec<Uuid>,
    ...
    .bind(id)
    .bind(Some(Uuid::from_str(
        "c53954ff-13aa-412c-844a-b97faca10ef6",
    )?))
    .bind(Some([
        Uuid::from_str("7d814b8f-1894-4b97-927c-83e82cb6735b")?,
        Uuid::from_str("f8e9f4c2-3f5d-4437-920a-8644efb72676")?,
...
async fn describe_uuid(pool: ScyllaDBPool) -> anyhow::Result<()> {
    ...
    let describe = conn
        .describe("SELECT my_id, my_uuid, my_uuid_list, my_uuid_set FROM uuid_tests".into_sql_str())
        .await?;
